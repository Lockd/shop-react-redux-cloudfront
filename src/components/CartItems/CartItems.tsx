...
export default function CartItems({items, isEditable}: CartItemsProps) {
  ...
  const totalPrice: number = items.reduce((total, item) => (item.count * item.product.priceUSD + total), 0);
      ...
      <List disablePadding>
        {items.map((cartItem: CartItem) => (
          <ListItem className={classes.listItem} key={cartItem.product.id}>
            ...
            <Typography
              variant="body2">{formatAsPrice(cartItem.product.priceUSD)} x {cartItem.count} = {formatAsPrice(cartItem.product.priceUSD * cartItem.count)}</Typography>
          </ListItem>
        ))}
